...
void main() {
  test('Artillery Barrage Lifecycle simulation', () {
    ...
    print('Alphonse spawn: $spawnedAlphonse');

    final barrage = CombatUnitFactory.createArtilleryBarrage();
    print('Barrage initial stats: unitType=${barrage.combatStats?.unitType}, deploymentTime=${barrage.combatStats?.deploymentTime}');

    final spawned = manager.spawnUnit(barrage, CombatSide.player, x: 50.0, y: 42.5);
    print('Spawn successful: $spawned');

    final combatant = manager.combatants.firstWhere((c) => c.npc.name.contains('Barrage'));
    print('Spawned Combatant: name=${combatant.npc.name}, activeDeploymentTimer=${combatant.activeDeploymentTimer}, supportDurationRemaining=${combatant.supportDurationRemaining}');
    ...
    for (int i = 1; i <= 14; i++) {
      ...
      if (list.isEmpty) {
        print('Tick $i (Time elapsed: ${i * 0.5}s): Barrage unit is dead/vanished!');
      } else {
        final c = list.first;
        print('Tick $i (Time elapsed: ${i * 0.5}s): activeDeploymentTimer=${c.activeDeploymentTimer}, supportDurationRemaining=${c.supportDurationRemaining}, isDead=${c.isDead}');
      }
    }
  });
}
